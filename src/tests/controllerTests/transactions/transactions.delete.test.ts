...
describe("/api/transactions/ [DELETE]", () => {
  ...
  it("deletes own resources", async (done) => {
    ...
    expect(before.t).toHaveLength(3);
    expect(before.t.find((_: any) => _.id === ids[0])).toBeDefined();
    expect(before.t.find((_: any) => _.id === ids[1])).toBeDefined();
    expect(before.t.find((_: any) => _.id === ids[2])).toBeDefined();

    const response = await client.transactions().delete(ids[0]);
    expect(response.status).toBe(204);

    const after = await (await client.transactions().get()).json();
    expect(after.t.find((_: any) => _.id === ids[0])).toBeUndefined();
    expect(after.t.find((_: any) => _.id === ids[1])).toBeDefined();
    expect(after.t.find((_: any) => _.id === ids[2])).toBeDefined();
    done();
  });
  ...
  it("cannot delete other people's resources", async (done) => {
    ...
    expect(response1.t).toHaveLength(1);
    ...
    expect(response2.t).toHaveLength(1);
    ...
    expect(response3.t).toHaveLength(0);
    done();
  });
});
